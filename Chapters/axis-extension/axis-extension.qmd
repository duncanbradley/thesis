...
# then convert back to a formula
  ...
  format_formula() %>%
  str_replace_all(c("mag_slider.response" = "magnitude_rating", 
...
#| include: false
random_str(e1_mag)
xx <- paste(print_formula(e1_mag))
```

This model employed a maximal random effects structure, capturing the baseline responses (intercepts) and differences between the two axis settings (slopes) separately for each individual participant and each individual item. The model formula was as follows: \``r xx`\`
