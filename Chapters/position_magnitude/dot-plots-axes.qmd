...
summary_extract <- function(model, key_term) {
  ...
  one_row <- tidy(model) %>% filter(term == key_term)
    ...
    assign(value = one_row %>% pull(param),
           ...
           paste0(model_name, 
                  ".", 
                str_c(unlist(str_extract_all("pos1:ori1:literacy", "\\b\\w")), collapse = ""),
                ".",
                  param))
    }
...
summary_extract(E1_clint, "condition1")

# summary output for E1 severity rating model with literacy interation
summary_extract(E1_slint, "condition1")
```
...
Additional models explored the *interaction between* literacy and physical position. For magnitude ratings, there was a significant interaction: 
