...
The odds ratio for the difference between conditions is `r printnum(exp(E1_c.estimate))` (`r print_confint(exp(E1_c.CI))`). Participants were `r printnum(exp(E1_c.estimate))` times more likely to respond with a higher magnitude rating to data points presented at high positions than data points presented at low positions. This is equivalent to a Cohen's d value of `r printnum(abs(oddsratio_to_d(exp(E1_c.estimate))))`, a `r paste(interpret_cohens_d(oddsratio_to_d(exp(E1_c.estimate))))` effect size.
...
The odds ratio for the difference between conditions is `r printnum(exp(E1_s.estimate))`, `r print_confint(exp(E1_s.CI))`. Participants were `r printnum(exp(E1_s.estimate))` times more likely to respond with a higher severity rating to data points presented at high positions than data points presented at low positions. This is equivalent to a Cohen's d value of `r printnum(abs(oddsratio_to_d(exp(E1_s.estimate))))`, a `r paste(interpret_cohens_d(oddsratio_to_d(exp(E1_s.estimate))))` effect size.
...
emtrends(E1_clint, pairwise ~ condition, var = "literacy")

emmip(E1_clint, condition ~ literacy, cov.reduce = range)
```
...
emtrends(E1_slint, pairwise ~ condition, var = "literacy")

emmip(E1_slint, condition ~ literacy, cov.reduce = range)
```
...
include_graphics("images/fig-rationale-1.png")
...
Two scenarios which were fillers in E1 were used as experimental scenarios[^3] and three additional scenarios were created. One filler scenario was removed due to a concern about its quality (it concerned the risk to others as well as the risk to oneself). This resulted in a total of 24 experimental scenarios, 12 filler scenarios, and 5 attention check questions (41 trials in total).
...
L2 <- likert_plot(E2_tidy) + facet_wrap(~ ori, ncol = 1, labeller = labeller(ori = str_to_title)) +
  geom_segment(x=100, y=4.4, xend=650, yend=4.4,
               arrow=arrow(ends = "both", type = "closed", length = unit(10, "pt")), data = E2_tidy %>% slice_head(n = 1)) +
...
A likelihood ratio test reveals that a model including the interaction between physical position and axis orientation as a fixed effect explains significantly more variability in ratings than a model without this interaction as a fixed effect ($\chi^2$(`r in_paren(E2_c.df)`) = `r printnum(E2_c.LR)`, p `r printp(E2_c.p, add_equals = TRUE)`). There was a significant interaction between physical position and y-axis orientation (z = `r printnum(abs(E2_c.statistic))`, p `r printp(E2_c.p.value, add_equals = TRUE)`).

The odds ratio associated with this interaction is `r printnum(exp(E2_c.estimate))` (`r print_confint(exp(E2_c.CI))`). This is equivalent to a Cohen's d value of `r printnum(abs(oddsratio_to_d(exp(E2_c.estimate))))`, a `r paste(interpret_cohens_d(oddsratio_to_d(exp(E2_c.estimate))))` effect size.
...
For ratings of the severity of consequences, a likelihood ratio test reveals that a model including the interaction between physical position and axis orientation as a fixed effect explains significantly more variability in ratings than a model without this interaction as a fixed effect ($\chi^2$(`r in_paren(E2_s.df)`) = `r printnum(E2_s.LR)`, p `r printp(E2_s.p, add_equals = TRUE)`). There was a significant interaction between physical position and y-axis orientation (z = `r printnum(abs(E2_s.statistic))`), p `r printp(E2_s.p.value, add_equals = TRUE)`).

The odds ratio associated with this interaction is `r printnum(exp(E2_s.estimate))` (`r print_confint(exp(E2_s.CI))`). This is equivalent to a Cohen's d value of `r printnum(abs(oddsratio_to_d(exp(E2_s.estimate))))`, a `r paste(interpret_cohens_d(oddsratio_to_d(exp(E2_s.estimate))))` effect size.
...
emtrends(E2_clint, pairwise ~ pos:ori, var = "literacy")

emmip(E2_clint, pos:ori ~ literacy, cov.reduce = range)
```
...
#| include: false
# print table showing model random effects structure (not used in final document)
add_header_above(kable(random_str(E3_c), "latex"), "E3_c")
```
...
A likelihood ratio test reveals that a model including physical position as a fixed effect explains significantly more variability in ratings than a model which does not include physical position as a fixed effect ($\chi^2$(`r in_paren(E3_c.df)`) = `r printnum(E3_c.LR)`, p `r printp(E3_c.p, add_equals = TRUE)`). Data points' magnitudes were rated as larger when those data points were presented at *low* physical positions, compared to when the same data points were presented at high physical positions, in contrast to the findings in Experiment 1 (z = `r printnum(abs(E3_c.statistic))`, p `r printp(E3_c.p.value, add_equals = TRUE)`).

The odds ratio for the difference between conditions is `r printnum(1/exp(E3_c.estimate))` (`r print_confint(rev(1/exp(E3_c.CI)), conf.int = 0.95)`). Participants were `r printnum(1/exp(E3_c.estimate))` times more likely to respond with a higher magnitude rating to data points presented at *low* positions than data points presented at high positions. This is equivalent to a Cohen's d value of `r printnum(abs(oddsratio_to_d(1/exp(E3_c.estimate))))`, a `r paste(interpret_cohens_d(oddsratio_to_d(exp(E3_c.estimate))))` effect size.
...
#| include: false
# print table showing model random effects structure (not used in final document)
add_header_above(kable(random_str(E3_s), "latex"), "E3_s")
```

For ratings of the severity of consequences, a likelihood ratio test reveals that a model including physical position as a fixed effect explained numerically more variability in ratings than a model which does not include condition as a fixed effect, although this failed to reach significance: ($\chi^2$(`r in_paren(E3_s.df)`) = `r printnum(E3_s.LR)`, p `r printp(E3_s.p, add_equals = TRUE)`). The severity of consequences was rated as numerically greater when data points representing the chance of an event occurring were presented at low physical positions, compared to when the same data points were presented at high physical positions, although this failed to reach significance (z = `r printnum(abs(E3_s.statistic))`, p `r printp(E3_s.p.value, add_equals = TRUE)`).

The odds ratio for the difference between conditions is `r printnum(1/exp(E3_s.estimate))` (`r print_confint(rev(1/exp(E3_s.CI)), conf.int = 0.95)`). Participants were `r printnum(1/exp(E3_s.estimate))` times more likely to respond with a higher severity rating to data points presented at low positions than data points presented at high positions. This is equivalent to a Cohen's d value of `r printnum(abs(oddsratio_to_d(1/exp(E3_s.estimate))))`, a `r paste(interpret_cohens_d(oddsratio_to_d(exp(E3_s.estimate))))` effect size.
...
Adjusting for participants' data visualisation literacy scores did not change the pattern of results regarding ratings of the magnitude of data points themselves (z = `r printnum(abs(E3_cl.statistic))`, p `r printp(E3_cl.p.value, add_equals = TRUE)`, odds ratio = `r printnum(1/exp(E3_cl.estimate))`, `r print_confint(rev(1/exp(E3_cl.CI)), conf.int = 0.95)`) or the severity of consequences (z = `r printnum(abs(E3_sl.statistic))`, p `r printp(E3_sl.p.value, add_equals = TRUE)`, odds ratio = `r printnum(1/exp(E3_sl.estimate))`, `r print_confint(rev(1/exp(E3_sl.CI)), conf.int = 0.95)`).
...
emtrends(E3_clint, pairwise ~ condition, var = "literacy")

emmip(E3_clint, condition ~ literacy, cov.reduce = range)
```
...
emtrends(E3_slint, pairwise ~ condition, var = "literacy")

emmip(E3_slint, condition ~ literacy, cov.reduce = range)
```
