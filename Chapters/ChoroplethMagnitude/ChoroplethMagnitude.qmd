...
Different interpretations of the same dataset can also arise through modified displays of the same colour scale. Empirical research has compared colour legends which only indicate uncertainty using colour features (e.g., increasing luminance and decreasing saturation), to colour legends which also signal uncertainty through increasing reduction in the range of possible colours, termed Value-Suppressing Uncertainty Palettes [VSUPs, @correll_value-suppressing_2018]. In Correll et al.'s study, participants played a 'Battleship' style game which involved reducing risk by balancing danger and uncertainty. Participants were more likely to favour riskier but more certain options over uncertain options when using VSUPs. Constraining the range of colours at higher uncertainty levels may have reduced the impression that these data points could represent desirable low-danger magnitudes. The experiment we report below examines directly how the range of values in a colour legend (and thus the degree of contrast) affects interpretations of magnitude.

## Methodology

### Outline

The present experiment investigates the influence of colour legend range on the cognitive processing of magnitude. In one condition, the colour legend's upper bound was equal to the maximum plotted value (*truncated range*), in the other, it was equal to double the maximum plotted value (*extended range*). The data, colour palette, geographic regions, and the mapping between colours and numerical values, were identical across conditions. Therefore, manipulating the colour legend's range did not modify the luminance profile of the map itself. Instead, the overall *contrast* differed: an extended legend included values with lower luminance than those in the map, a truncated legend did not. Overall contrast is intrinsically linked to the range of the colour legend. Example stimuli are shown in @fig-example-stimuli.

We employ the term 'truncated' in a broad sense, referring to a scale that is constrained such that potentially relevant values are omitted, not simply a scale that excludes a zero value. Using a lower bound of zero reduced the number of differences between the two conditions, so that only the upper bound was manipulated. This also meant that plotted values' variability appeared smaller, assisting participants in judging the *overall* magnitude of these values.

Rather than asking participants to make abstract judgements about the size of abstract values, we presented fictitious pollution data, and asked how urgently action should be taken to address the pollution levels displayed in each data visualisation. Measuring urgency provides a proxy of participants' latent magnitude representations, capturing interpretations through the type of judgements which can drive behaviour. In addition to increased ecological validity, we also anticipated that pollution data might be able to generate a balanced set of responses to the question of urgency. A variable evoking an extreme negative reaction may have elicited responses at ceiling and one too trivial may have elicited responses at floor. We expected participants to recognise that a sufficient degree of pollution would require action, but also understand that low levels may require less urgent action.

Participants responded using a scale with "Not very urgently" at one extreme and "Very urgently" at the other. We did not provide a specific definition of urgency for participants to use when making their responses. Overly specific guidance on interpretation of data in unfamiliar units would inordinately influence participants' responses, inhibiting spontaneous judgements. Therefore, different participants' responses may reflect different notions of urgency. However, the within-participants design accounts for individual variation. Each participant's ratings are compared against their own ratings for the alternative condition, allowing for meaningful comparison between conditions for each participant.

Pollution levels were displayed in choropleth maps, which use colour encoding to display data aggregated at the level of geographic areas. Six different colour palettes were employed throughout the experiment to increase variation across trials (@fig-example-stimuli). However, hue was consistent across conditions for each individual item, therefore the choropleth maps conveyed values through luminance. Note that we do not consider the designs of choropleth maps in this experiment to reflect best practice for plotting pollution statistics. Rather, these designs were motivated by the desire to examine the role of colour legends in the interpretation of magnitude. Previous research has illustrated that the size of geographical regions can influence ensemble coding in choropleth maps [@schiewe_empirical_2019]. However, we did not control for this aspect, instead we prioritised ecological validity by using maps with real geographical regions. These maps appeared identical across conditions in order to avoid this bias confounding results.
...
#| fig-width: 4
#| out-width: 450px
#| fig-scap: "Example stimuli: choropleth maps showing fictitious pollution data."
#| fig-cap: "Example stimuli: choropleth maps showing fictitious pollution data. Within each row, the map is identical. The columns show the different experimental conditions: the four types of legend accompanying each map. In the top three rows, the maximum plotted value in each map is 200, so the highest value in the truncated legend is 200 and the highest value in the extended legend is 400 (double the maximum plotted value). In the bottom three rows, the maximum plotted value in each map is 900, so the highest value in the truncated legend is 900 and the highest value in the extended legend is 1800 (double the maximum plotted value). During the experiment, all six colour palettes were used in conjunction with all maximum plotted values."
#| fig-alt: "A 4x6 grid of choropleth map visualisations, in six different colours, each with an accompanying legend. The colour legends below the maps in the left columns terminate with the same shade of colour as the darkest geographical region. The colour legends below the maps in the right columns terminate with a much darker shade of colour. Each colour legend is shown with and without numerical labels."
reds_200a <- ggplot() + background_image(image_read("data/supermap_stimuli/E1_extend_blank.png")) + coord_fixed(ratio = 1) + theme(plot.margin = unit(c(0,0,0,0), "cm"))
reds_200b <- ggplot() + background_image(image_read("data/supermap_stimuli/E1_extend_label.png")) + coord_fixed(ratio = 1) + theme(plot.margin = unit(c(0,0,0,0), "cm"))
reds_200c <- ggplot() + background_image(image_read("data/supermap_stimuli/E1_trunc_blank.png")) + coord_fixed(ratio = 1) + theme(plot.margin = unit(c(0,0,0,0), "cm"))
reds_200d <- ggplot() + background_image(image_read("data/supermap_stimuli/E1_trunc_label.png")) + coord_fixed(ratio = 1) + theme(plot.margin = unit(c(0,0,0,0), "cm"))
...
grey_200a <- ggplot() + background_image(image_read("data/supermap_stimuli/E2_extend_blank.png")) + coord_fixed(ratio = 1) + theme(plot.margin = unit(c(0,0,0,0), "cm"))
grey_200b <- ggplot() + background_image(image_read("data/supermap_stimuli/E2_extend_label.png")) + coord_fixed(ratio = 1) + theme(plot.margin = unit(c(0,0,0,0), "cm"))
grey_200c <- ggplot() + background_image(image_read("data/supermap_stimuli/E2_trunc_blank.png")) + coord_fixed(ratio = 1) + theme(plot.margin = unit(c(0,0,0,0), "cm"))
grey_200d <- ggplot() + background_image(image_read("data/supermap_stimuli/E2_trunc_label.png")) + coord_fixed(ratio = 1) + theme(plot.margin = unit(c(0,0,0,0), "cm"))

purp_200a <- ggplot() + background_image(image_read("data/supermap_stimuli/E3_extend_blank.png")) + coord_fixed(ratio = 1) + theme(plot.margin = unit(c(0,0,0,0), "cm"))
purp_200b <- ggplot() + background_image(image_read("data/supermap_stimuli/E3_extend_label.png")) + coord_fixed(ratio = 1) + theme(plot.margin = unit(c(0,0,0,0), "cm"))
purp_200c <- ggplot() + background_image(image_read("data/supermap_stimuli/E3_trunc_blank.png")) + coord_fixed(ratio = 1) + theme(plot.margin = unit(c(0,0,0,0), "cm"))
purp_200d <- ggplot() + background_image(image_read("data/supermap_stimuli/E3_trunc_label.png")) + coord_fixed(ratio = 1) + theme(plot.margin = unit(c(0,0,0,0), "cm"))

blue_900a <- ggplot() + background_image(image_read("data/supermap_stimuli/E46_extend_blank.png")) + coord_fixed(ratio = 1) + theme(plot.margin = unit(c(0,0,0,0), "cm"))
blue_900b <- ggplot() + background_image(image_read("data/supermap_stimuli/E46_extend_label.png")) + coord_fixed(ratio = 1) + theme(plot.margin = unit(c(0,0,0,0), "cm"))
blue_900c <- ggplot() + background_image(image_read("data/supermap_stimuli/E46_trunc_blank.png")) + coord_fixed(ratio = 1) + theme(plot.margin = unit(c(0,0,0,0), "cm"))
blue_900d <- ggplot() + background_image(image_read("data/supermap_stimuli/E46_trunc_label.png")) + coord_fixed(ratio = 1) + theme(plot.margin = unit(c(0,0,0,0), "cm"))
  
gree_900a <- ggplot() + background_image(image_read("data/supermap_stimuli/E47_extend_blank.png")) + coord_fixed(ratio = 1) + theme(plot.margin = unit(c(0,0,0,0), "cm"))
gree_900b <- ggplot() + background_image(image_read("data/supermap_stimuli/E47_extend_label.png")) + coord_fixed(ratio = 1) + theme(plot.margin = unit(c(0,0,0,0), "cm"))
gree_900c <- ggplot() + background_image(image_read("data/supermap_stimuli/E47_trunc_blank.png")) + coord_fixed(ratio = 1) + theme(plot.margin = unit(c(0,0,0,0), "cm"))
gree_900d <- ggplot() + background_image(image_read("data/supermap_stimuli/E47_trunc_label.png")) + coord_fixed(ratio = 1) + theme(plot.margin = unit(c(0,0,0,0), "cm"))
  
oran_900a <- ggplot() + background_image(image_read("data/supermap_stimuli/E48_extend_blank.png")) + coord_fixed(ratio = 1) + theme(plot.margin = unit(c(0,0,0,0), "cm"))
oran_900b <- ggplot() + background_image(image_read("data/supermap_stimuli/E48_extend_label.png")) + coord_fixed(ratio = 1) + theme(plot.margin = unit(c(0,0,0,0), "cm"))
oran_900c <- ggplot() + background_image(image_read("data/supermap_stimuli/E48_trunc_blank.png")) + coord_fixed(ratio = 1) + theme(plot.margin = unit(c(0,0,0,0), "cm"))
oran_900d <- ggplot() + background_image(image_read("data/supermap_stimuli/E48_trunc_label.png")) + coord_fixed(ratio = 1) + theme(plot.margin = unit(c(0,0,0,0), "cm"))

supermap <- 
  (reds_200c|reds_200d|reds_200a|reds_200b) / 
  (grey_200c|grey_200d|grey_200a|grey_200b) / 
  (purp_200c|purp_200d|purp_200a|purp_200b) / 
  (blue_900c|blue_900d|blue_900a|blue_900b) / 
  (gree_900c|gree_900d|gree_900a|gree_900b) / 
  (oran_900c|oran_900d|oran_900a|oran_900b)
supermap
```
...
The experiment was programmed using PsychoPy [@peirce_psychopy2_2019, version 2022.1.4] and hosted on pavlovia.org. A link to an interactive version of this experiment is available in this project's online repository: <https://osf.io/5fg8k/>. Participants were instructed to use laptop or desktop computers, rather than another type of device, and were told that the experiment was about using information to make decisions. Participants were informed that in each map, each region's colour reflected its pollution level, and that data on different types of pollution were shown throughout the experiment, with pollution levels presented using standardised units. We did not calibrate or measure colour display on participants' own screens, meaning the stimuli would not appear identical for different participants. However, using a within-participants design prevents these differences from impairing our analysis. Each participant was exposed to all experimental conditions under the same display conditions. Therefore, an individual participant's responses to each condition can be compared to each other without display conditions acting as a confounding variable.
...
For each item, a 'mappable' object defined the mapping between numerical values and colours for both truncated and extended colour legends. The lightest colour in the scale was mapped to zero and the darkest colour to double the maximum value. This range was employed in the extended colour legend. The truncated colour legend, on the other hand, terminated at the maximum value in the data, so the range was halved and therefore overall contrast was lower (but the mapping between numerical values and colours was retained). No classification was employed in the legends, for maximum consistency across conditions. Where numerical labels were present, an identical number of labels (between six and ten) appeared on both versions of a colour legend. Tick marks were absent from all colour legends.
...
This suggests that the numerical labels themselves were not responsible for the effect of maximum value. Instead, this effect may have been driven by the amount of contrast within the choropleth map. The colour for the maximum value was identical in each map with the same colour palette, but the three *accompanying* values in each map were always between 10 and 30 units less than the maximum value. Consequently, these values were represented by darker colours when the maximum value was higher, thus conveying greater overall magnitude. Colour legend range ($\eta_p^2$ `r print_es(full_max_value_int1.eta.range)`) remains a greater influence than maximum value ($\eta_p^2$ `r print_es(full_max_value_int1.eta.max_value)`).
...
Choropleth maps are typically used to convey spatial variability, but may alternatively be employed to convey overall magnitude. This experiment clearly demonstrates that the range of the accompanying colour legend influences interpretations of absolute magnitude in such choropleth maps.

Overall contrast is increased whenever a colour legend extends beyond the range of plotted data. In this experiment, extending the numerical range of the colour legend beyond the maximum plotted value introduced lower luminance values, providing greater contrast. Consequently, participants rated the urgency of addressing pollution levels as lower, compared to when the colour legend terminated with the maximum plotted value. This illustrates that viewers use colour legends to put numbers' magnitudes into perspective, interpreting magnitude with respect to the range of the colour legend. A colour legend does not only provide a mapping between numerical values and colours, it also provides a range of values relevant for considering the absolute magnitude of presented data.
...
Colour legends simultaneously encode changes in number through both colour and physical position. Different values are represented by different colours *and* occupy different positions on the colour legend. In the present experiment, plotted values' analogous positions in the *truncated* colour legend were on the far right hand side, and their corresponding colours had the same luminance as those at the extreme of the legend. On the other hand, plotted values' analogous positions were in the middle of the *extended* colour legend, and their corresponding colours had higher luminance than those at the extreme of the legend. This experiment cannot determine whether the location of plotted values on the legend, the range of colours included in the legend, or both of these factors, influenced processing of magnitude. The manipulation of numerical labels does not assist in answering this question because colour legends still encode changes in number even when these changes are not labelled. However, this question may have little practical relevance since these aspects are intrinsically linked in a typical colour legend.
