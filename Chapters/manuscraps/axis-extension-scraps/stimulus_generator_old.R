...
item_no <- 1:46 # numerical identifiers for each item

item_type <- c(rep('E', 32), rep('F', 8), rep('AC', 6)) # 32 Experimental, 8 Filler, 6 Attention Check
...
prop <- c(rep(c(.2, .4), each = 16), # in experimental items, the mean for the sampling distribution will be 20% or 40% of upper_lim
          rep(.9, times = 8), # in filler items, the mean for the sampling distribution will be 90% of upper_lim
...
correct_response <- c(rep(NA, 40), 1, 1, 1.5, 1.5, 2, 2) # correct response for attention check questions

granularity_val <- c(rep(0, 40), rep(0.5, 6)) # granularity of reponse slider
